...
export function TrendView({ result }: TrendViewProps) {
  ...
  const mod = result.detailSurface?.modules.trend;
  const allPoints = mod?.data.points ?? [];
  ...
  const trendData = allPoints.slice(-months);
  ...
  const derivedMonthlyAverageChange = useMemo(() => {
    if (allPoints.length < 2) return benchmark.momChange;
    const changes = allPoints
      .slice(1)
      .map((point, index) => {
        const previous = allPoints[index]?.p50 ?? 0;
  ...
  }, [allPoints, benchmark.momChange]);

  const derivedYoyChange = useMemo(() => {
    if (allPoints.length < 2) return benchmark.yoyChange;
    const first = allPoints[0]?.p50 ?? 0;
    const last = allPoints[allPoints.length - 1]?.p50 ?? 0;
    if (!first) return benchmark.yoyChange;
    return ((last - first) / first) * 100;
  }, [allPoints, benchmark.yoyChange]);
  ...
  return (
    <div className="bench-section">
      ...
      {isLoading ? (
        <ModuleStateBanner variant="loading" message="Loading trend data..." className="mb-4" />
      ) : null}
      {!hasTrendData && !isLoading && (
        <div className="mb-4 rounded-xl bg-amber-50 px-4 py-3 text-xs text-amber-700">
          {mod?.message ?? "Historical trend points are not available for this benchmark row yet."}
        </div>
      )}
      ...
      <div className="flex items-center gap-4 rounded-xl bg-brand-50 px-4 py-3 mb-4">
        ...
        <div>
          ...
          <p className="text-lg font-bold text-brand-900">{formatAED(endValue)}</p>
        </div>
        <div className="h-10 w-px bg-brand-200" />
        <div>
          <p className="text-xs font-medium text-brand-600">Start</p>
          <p className="text-sm font-semibold text-brand-700">{formatAED(startValue)}</p>
        </div>
      </div>
        ...
        <div className="mt-2 h-[280px] min-w-0">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={chartData} margin={{ top: 8, right: 8, left: 8, bottom: 0 }}>
              <CartesianGrid stroke={PAYROLL_TREND_CHART_STYLE.gridColor} strokeWidth={1} vertical={false} />
              <XAxis
                dataKey="month"
                axisLine={false}
                tickLine={false}
              ...
              />
              <Area type="monotone" dataKey="P25" stroke="#9CA3AF" strokeWidth={2} fillOpacity={0} dot={buildDot("#9CA3AF")} activeDot={buildActiveDot("#9CA3AF")} />
              <Area type="monotone" dataKey="Median" stroke={PAYROLL_TREND_CHART_STYLE.lineColor} strokeWidth={PAYROLL_TREND_CHART_STYLE.lineWidth} fillOpacity={0} dot={buildDot(PAYROLL_TREND_CHART_STYLE.lineColor)} activeDot={buildActiveDot(PAYROLL_TREND_CHART_STYLE.lineColor)} />
              <Area type="monotone" dataKey="P75" stroke="#4B5563" strokeWidth={2} fillOpacity={0} dot={buildDot("#4B5563")} activeDot={buildActiveDot("#4B5563")} />
            </AreaChart>
          </ResponsiveContainer>
      ...
      <div className="flex flex-wrap gap-2 mt-4 pt-4 border-t border-border">
        <div className="flex items-center gap-2 rounded-full bg-brand-100 px-3 py-1.5">
          ...
          <span className="text-xs font-medium text-brand-700">
            Monthly avg: {derivedMonthlyAverageChange > 0 ? "+" : ""}
            {derivedMonthlyAverageChange.toFixed(1)}%
          </span>
        </div>
        <div
          className={clsx(
        ...
        >
          <span className={clsx("h-2 w-2 rounded-full", derivedYoyChange >= 0 ? "bg-emerald-500" : "bg-red-500")} />
          <span className={clsx("text-xs font-medium", derivedYoyChange >= 0 ? "text-emerald-700" : "text-red-700")}>
            YoY: {derivedYoyChange > 0 ? "+" : ""}
            {derivedYoyChange.toFixed(1)}%
          </span>
        </div>
      </div>
...
function TrendTooltip({
  ...
  label,
  formatValue,
}: TrendTooltipProps & { formatValue: (value: number) => string }) {
