...
export function CompliancePresetSwitcher({
  currentPresetId,
  isCustomized,
  onSelectPreset,
  onReset,
}: CompliancePresetSwitcherProps) {
  const [isOpen, setIsOpen] = useState(false);
  const dropdownRef = useRef<HTMLDivElement>(null);

  const currentPreset = COMPLIANCE_PRESETS[currentPresetId];
  const CurrentIcon = currentPreset ? PRESET_ICONS[currentPreset.id] || Layout : Layout;
    ...
    <div className="relative" ref={dropdownRef}>
      ...
      {isOpen && (
        <div className="absolute left-0 top-full z-50 mt-2 w-72 rounded-2xl border border-border/50 bg-white p-2 shadow-xl">
          ...
          {Object.values(COMPLIANCE_PRESETS).map(preset => {
            const PresetIcon = PRESET_ICONS[preset.id] || Layout;
            return (
              <button
                key={preset.id}
                type="button"
